...
export default function RelatoriaForm({ id }: { id?: string } = {}) {
  ...
  const [formData, setFormData] = useState({
    ...
    descricao: '',
    ementa: '',
    tipo: 'Relatoria de Lei Ordinária (PL)',
    autor: '',
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (isEditing) {
        const { data: relatoriaData } = await supabase.from('relatorias').select('*').eq('id', resolvedParams.id).single();
        if (relatoriaData) {
          const nomeAutor = relatoriaData.autor || selectedDeputado?.nome || '';
          const autorCorrespondente = listaAutoresCarregados.find(
            (a: AutorItem) => (relatoriaData.id_autor && a.id === relatoriaData.id_autor) || a.nome.toLowerCase() === nomeAutor.toLowerCase()
          );

          setFormData({
            data: relatoriaData.data || new Date().toISOString().split('T')[0],
            descricao: relatoriaData.descricao || '',
            ementa: relatoriaData.ementa || '',
            tipo: relatoriaData.tipo || 'Relatoria de Lei Ordinária (PL)',
            autor: nomeAutor,
            id_autor: autorCorrespondente?.id && !autorCorrespondente.id.startsWith('legacy-') ? autorCorrespondente.id : (relatoriaData.id_autor || null),
            etapa: relatoriaData.etapa || 'Liberado',
            tramitacao: relatoriaData.tramitacao || 'Em elaboração',
            url_legislativo: relatoriaData.url_legislativo || '',
            numero_proposicao: relatoriaData.numero_proposicao || ''
          });

          // Buscar áreas temáticas associadas
          const { data: relations } = await supabase
            .from('relatoria_areas')
            .select('id_area_tematica')
            .eq('id_relatoria', resolvedParams.id);
          
          if (relations && relations.length > 0) {
            setSelectedAreaIds(relations.map((r: any) => r.id_area_tematica));
          } else if (relatoriaData.id_area_tematica) {
            setSelectedAreaIds([relatoriaData.id_area_tematica]);
          }
        }
        setFetching(false);
      } else {
  ...
  const handleSalvarNovoAutor = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!error && insertedAutor) {
        ...
        setIsNovoAutorModalOpen(false);
        setAutorNotice(`Autor "${payload.nome}" selecionado para o relatoria com sucesso.`);
      }
    } catch (err) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (isEditing) {
      let { error } = await supabase.from('relatorias').update(finalPayload).eq('id', resolvedParams.id);
      ...
      if (error && (error.message?.includes('id_autor') || error.code === 'PGRST204')) {
        ...
        const retry = await supabase.from('relatorias').update(finalPayload).eq('id', resolvedParams.id);
        error = retry.error;
      }

      if (!error) {
        try {
          await supabase.from('relatoria_areas').delete().eq('id_relatoria', resolvedParams.id);
          if (selectedAreaIds.length > 0) {
            const relations = selectedAreaIds.map(areaId => ({
              id_relatoria: resolvedParams.id,
              id_area_tematica: areaId
            }));
            await supabase.from('relatoria_areas').insert(relations);
          }
        } catch (err) {
          console.error('Error saving projects-areas relations:', err);
        }
        router.push('/relatoria');
      } else {
        alert('Erro ao atualizar relatoria: ' + (error.message || ''));
      }
    } else {
      let { data: newRel, error } = await supabase.from('relatorias').insert([finalPayload]).select().single();
      
      // Fallback gracioso se a coluna id_autor ainda não existir na tabela remota do Supabase
      if (error && (error.message?.includes('id_autor') || error.code === 'PGRST204')) {
        delete finalPayload.id_autor;
        const retry = await supabase.from('relatorias').insert([finalPayload]).select().single();
        newRel = retry.data;
        error = retry.error;
      }
      ...
      if (!error && newRel) {
        ...
        alert('Erro ao salvar relatoria: ' + (error ? error.message : ''));
      }
    }
    setLoading(false);
  }
  ...
  return (
    <div className="p-8 space-y-8 max-w-4xl mx-auto">
      <div className="flex items-center gap-4">
        <Link href="/relatoria" className="p-2 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">
          <ArrowLeft size={20} className="text-slate-600" />
        </Link>
        <div>
          <p className="text-sm font-bold text-primary uppercase tracking-widest mb-1">Relatorias</p>
          <h2 className="text-3xl font-black font-headline text-on-surface">
            {isEditing ? 'Editar Relatoria' : 'Nova Relatoria'}
          </h2>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-xl shadow-sm border border-slate-100 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="col-span-2 space-y-2">
            <label className="text-xs font-bold uppercase tracking-widest text-on-surface-variant">Descrição da Relatoria</label>
            <input 
              required
              type="text" 
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold uppercase tracking-widest text-on-surface-variant">Tipo da Relatoria</label>
            <select 
              required
            ...
            >
              ...
              <option value="Relatoria de Lei Complementar (PLP)">Relatoria de Lei Complementar (PLP)</option>
              <option value="Relatoria de Lei Ordinária (PL)">Relatoria de Lei Ordinária (PL)</option>
              <option value="Relatoria de Decreto Legislativo (PDL)">Relatoria de Decreto Legislativo (PDL)</option>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              >
                <option value="">Selecione o autor da relatoria...</option>
          ...
          <div className="space-y-2 col-span-1 md:col-span-2">
            ...
            <p className="text-xs text-slate-500 mb-2">Selecione uma ou mais áreas temáticas para este relatoria:</p>
          ...
          <div className="space-y-2 col-span-1 md:col-span-2">
            <label className="text-xs font-bold uppercase tracking-widest text-on-surface-variant">Link da URL do Relatoria Legislativo</label>
            <input 
              type="url" 
              className="w-full bg-surface-container-low border border-transparent focus:border-primary/40 focus:bg-white rounded-lg px-4 py-3 text-sm outline-none transition-all"
        ...
        <div className="pt-6 flex justify-end">
          ...
          >
            <Save size={18} />
            {loading ? 'Salvando...' : 'Salvar Relatoria'}
          </button>
        </div>
      </form>
          ...
          <div className="bg-white max-w-md w-full rounded-2xl shadow-2xl p-6 space-y-5 border border-slate-100">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-base font-bold text-slate-900">Cadastrar Autor da Relatoria</h3>
