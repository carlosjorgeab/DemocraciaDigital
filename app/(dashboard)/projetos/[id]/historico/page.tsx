...
export default function HistoricoProjetoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabase.from('historico_projetos').insert([payload]);
  ...
  async function handleDelete(id: string) {
    if (confirm('Tem certeza que deseja excluir este registro?')) {
      await supabase.from('historico_projetos').delete().eq('id', id);
    ...
    <div className="p-8 space-y-8 max-w-5xl mx-auto">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <h2 className="text-3xl font-black font-headline text-on-surface">Histórico do Projeto</h2>
          <p className="text-on-surface-variant text-sm mt-1">
            <strong>Projeto:</strong> {projeto.descricao} | <strong>Município:</strong> {projeto.municipio || '-'}
          </p>
        </div>
      </div>
