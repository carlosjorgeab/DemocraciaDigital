...
export default function RelatoriasPublicPage() {
    ...
    async function fetchData() {
      ...
      try {
        const { data: rels, error: relError } = await supabase
          ...
          .select('id_area_tematica, areas_tematicas(id, nome, cor, icone_url), relatorias!inner(id_deputado, etapa)')
      ...
      try {
        ...
        if (projsData && projsData.length > 0) {
          const enrichedRelatorias = await Promise.all(projsData.map(async (p: any) => {
  ...
  const filteredRelatórias = relatorias.filter(p => {
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-12">
      ...
      <section>
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-2xl font-black uppercase text-slate-900 tracking-tight">Relatórias por Áreas Temáticas</h1>
            <p className="text-sm font-medium text-slate-500 mt-1">Selecione uma área temática para visualizar os relatorias relacionados do Deputado {selectedDeputado?.nome}.</p>
          </div>
        </div>
        ...
        {areas.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-3xl p-12 text-center shadow-sm">
            ...
            <p className="text-slate-500 max-w-md mx-auto">Este deputado ainda não possui áreas temáticas com relatorias liberados.</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
            {areas.map(area => (
              <button
                key={area.id}
                onClick={() => router.push(`/p/${selectedDeputado?.slug || selectedDeputado?.id}/relatorias/${area.id}`)}
      ...
      {/* 2. Listagem Geral de Relatórias */}
      <section className="pt-8 border-t border-slate-200">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-black uppercase text-slate-900 tracking-tight">Todos os Relatórias ({relatorias.length})</h2>
        ...
        {filteredRelatórias.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-3xl p-12 text-center shadow-sm">
            <div className="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-4 text-slate-400">
              <FileText size={28} />
            </div>
            <h3 className="text-base font-bold text-slate-900 mb-1">
              {searchTerm ? 'Nenhum relatoria encontrado para a busca' : 'Nenhum relatoria cadastrado'}
            </h3>
            <p className="text-xs text-slate-500">
              {searchTerm ? 'Tente pesquisar com outros termos.' : 'Não há relatorias liberados para visualização pública neste momento.'}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredRelatórias.map((relatoria) => {
              const primaryArea = relatoria.all_areas?.[0];
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-2 text-[11px] font-semibold text-slate-500 uppercase tracking-wider pt-3 border-t border-slate-100 mt-2">
                    {relatoria.autor ? (
                      <span className="truncate max-w-[200px]" title={relatoria.autor}>
                        Autor: <strong className="text-slate-700">{relatoria.autor}</strong>
                      </span>
                    ) : <span />}
